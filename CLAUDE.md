...
- **Todos os módulos de domínio do MVP (seção 4) estão implementados no backend e testados de ponta a ponta** (recebimento, produção, transferência, consumo, descarte, contagem) — **e o frontend já cobre todas as telas operacionais da seção 11**, só falta o Relatório de Movimentações (dado bruto navegável, sem prioridade — MVP não exige gráfico) e a tela de Usuários (hoje só dá pra criar login por `POST /usuarios` direto, sem UI). Falta também: `StockProvider` (Saipos/Varejo Fácil — precisa de detalhes reais da API, combinado com o usuário), `LabelPrinterProvider`.
