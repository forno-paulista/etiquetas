...
- **Auditoria (2026-09-25): já era assim.** Conferido em código — nenhum service de domínio importa SDK/HTTP client de terceiro, nenhuma lógica de negócio ramifica por nome de sistema externo. A única fresta era de nome: o enum `StatusAjusteExterno` tinha um valor `AJUSTADO_NO_ERP`, batizado do ponto de vista de uma integração que ainda nem existe — renomeado pra `AJUSTADO_EXTERNAMENTE` (migration `20260925120000_rename_status_ajuste_erp_neutro`). `IntegracaoExterna`/`MapeamentoProdutoExterno`/`SistemaExterno` continuam no schema sem nenhum código os usando — é exatamente o ponto de extensão que o `StockProvider` vai preencher quando for implementado, não uma violação de isolamento hoje. Não foi introduzida uma camada `domain/application/infrastructure` (arquitetura hexagonal "de livro") porque não há um segundo adapter pra justificar a interface — o próprio mecanismo de injeção de dependência do Nest (`StockProvider` como token, trocando a implementação) já entrega o isolamento real quando a integração existir; layers especulativas pra zero implementações hoje seriam over-engineering (ver "Princípio geral de projeto" acima).
...
- Rate limiting implementado (`@nestjs/throttler`): limite geral de 300 req/min por IP em toda a API (seção 15). O limite mais apertado que existia especificamente em `GET /lotes/qr/:qrCodeId` foi removido junto com a decisão de tornar o endpoint autenticado (seção 13) — não fazia mais sentido um limite à parte pra um endpoint que não é mais público. `POST /auth/login` tem o próprio limite, bem mais apertado (10/min) — login é alvo óbvio de força bruta.
- **Auditoria de segurança (2026-09-25, OWASP Top 10)** — o achado central: `LocalAccessGuard` só protegia rotas de escrita, os `GET`/list de `lotes`, `transferencias`, `descartes`, `ajustes-pendentes` e `relatorios/movimentos` não tinham guard nenhum — qualquer usuário autenticado via saldo/histórico de locais fora do próprio acesso, direto pela API (a UI já filtrava, mas isso não é fronteira de segurança). Corrigido:
  - `backend/src/common/local-scope.util.ts` (`localIdsPermitidos`): admin sem restrição; usuário comum sem filtro → restringe aos próprios `locaisAcesso`; usuário comum pedindo um `localId` fora do acesso → 403 (não silenciosamente vazio). Usado em `lotes`/`descartes`/`ajustes-pendentes`; `transferencias`/`relatorios` replicam a mesma lógica na forma OR (origem **ou** destino), já que não têm um único campo `localId`.
  - `GET /relatorios/movimentos` ganhou `@Roles(ADMIN, GESTOR_CD, GESTOR_LOJA)` — Operador não deveria ver relatório (seção 12) e antes via sem restrição nenhuma; frontend correspondente (`Layout.tsx`, `App.tsx`) escondeu o link/rota pro Operador também.
  - `POST /auth/login`: corrigido um timing side-channel (pedir um email que não existe pulava o `argon2.verify` inteiro, respondendo bem mais rápido que "senha errada" — dava pra enumerar emails cadastrados só pelo tempo de resposta). Agora sempre roda o verify, contra um hash fictício quando o usuário não existe.
  - Senha (criação e redefinição): além do `MinLength(8)` que já existia, agora exige letra e número (`usuarios/senha.validacao.ts`).
  - Auditoria (seção 7, tabela `Auditoria` — existia no schema, não era usada por ninguém): agora grava `LOGIN_SUCESSO`/`LOGIN_FALHOU` (sem nunca logar a senha) e `USUARIO_CRIADO`/`USUARIO_ATUALIZADO`/`SENHA_REDEFINIDA` com antes/depois de papel, status e locais de acesso. Uma falha ao gravar o log nunca derruba a ação em si.
  - `app.enableCors()` sem allowlist reflete qualquer origem — trocado por `CORS_ORIGIN` (env, csv de origens permitidas, default `http://localhost:5173`).
  - **Correção (2026-09-25, achada só quando o usuário pediu pra conferir os `.env`): existem DOIS `.env` diferentes e a correção acima, na primeira passada, só mexeu em um deles.** `backend/.env` é usado por ferramentas rodadas direto no host (`npm run db:seed`, Prisma CLI — por isso aponta pra `localhost:5432`); o `.env` da raiz é o que o `docker-compose.yml` de fato injeta no container do backend (aponta pra `postgres:5432`, hostname interno da rede Docker) — **é esse que importa em produção**, e continuou com `JWT_ACCESS_SECRET=change-me` e sem `CORS_ORIGIN` nenhum até essa correção (confirmado com `docker compose exec backend printenv` antes e depois). `docker-compose.yml` também exigia `JWT_REFRESH_SECRET` (`${JWT_REFRESH_SECRET:?defina...}`) — variável morta que nem o código lê — e nunca repassava `CORS_ORIGIN` pro container, o que teria deixado o fix de CORS sem efeito nenhum mesmo depois de setado no `.env`. Os dois `.env`/`.env.example` (raiz e `backend/`) agora têm secret forte gerado localmente + `CORS_ORIGIN`, sem `JWT_REFRESH_SECRET`; trocar o secret invalida sessões com access token já emitido, mas o refresh token não é JWT (não depende do secret) — quem já tava logado renova sozinho pelo interceptor do 401, sem precisar logar de novo.
  - Não mexido, por escolha: as condições de corrida no decremento de `SaldoLote` (`descarte`/`consumo`/`transferencias.enviar`/`producao` — read-then-write sem `WHERE quantidadeAtual >= X` nem isolamento elevado, ver auditoria arquitetural anterior) continuam abertas. É "Insecure Design" (A04), não estava no pedido explícito desta rodada, e mexer nos quatro services que movem saldo merece atenção própria (não é troca mecânica). Idem pro `npm audit` do backend (9 vulnerabilidades, 6 high — mas todas em dependência de build/CLI do Prisma, não em código que atende requisição; corrigir pede bump de major version do Prisma, testado à parte).
